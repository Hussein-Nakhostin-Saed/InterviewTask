...
where TViewModel : class
{
        ...
        string? orderBy, string? direction, string? search)
    {
        try
        {
            ...
            var query = MyDatabaseContext.Set<TModel>().CustomInclude(IncludableProperties).Where(current => current.IsDeleted == false);
    ...
    #region [HttpPost]
    //[HttpPost]
    protected virtual async Task<dynamic> Create(TModel entity)
    {
        try
        {
    ...
    #region [HttpPut("{id}")]
    //[HttpPut("{id}")]
    protected virtual async Task<dynamic> Update(Guid id, TModel entity)
    {
        try
        {
            if (id != entity.Id)
    ...
    protected virtual string[] IncludableProperties { get; }
}